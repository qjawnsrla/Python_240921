...
# 내부 젼수를 외부에서 접근하고자 할 떄 지정된 게터와 세터 메소드를 통해서만 접근하도록 하는 것
# private (__) : 같은 클래스 내에서만 접근 가능
# protected (_) : 같은 클래스 내이거나 상속 관계가 존재하면 접근 가능
# public(변수에 아무것도 안붙임) : 아무데서나 접근가능

class TV:
    cnt = 0 # 클래스 맴버
    def __init__(self, name, isOn, channel, volume):
        self.__name = name      # private 과 같은 기능 / private은 클래스 내부에서만 접근 가능 (접근제한자)
        self.__isOn = isOn
